 ...
 */
module wcwidth_cjk_compat.wcwidth;
...
private bool bisearch(uint ucs, immutable ref .interval[] table, size_t max)
...
private static immutable .interval[] combining =
[
	...
	{0x010A38, 0x010A3A}, {0x010A3F, 0x010A3F}, {0x01D167, 0x01D169},
	{0x01D173, 0x01D182}, {0x01D185, 0x01D18B}, {0x01D1AA, 0x01D1AD},
	{0x01D242, 0x01D244}, {0x0E0001, 0x0E0001}, {0x0E0020, 0x0E007F},
	{0x0E0100, 0x0E01EF},
];

private static immutable .interval[] not_apple =
[
	{0x3099, 0x309A},
];
...
public int wcwidth_ucs_compat(uint ucs, scope const wcwidth_cjk_compat_option* option = default_wcwidth_cjk_compat_option_ptr)
	...
	do
	{
		...
		if (!option.is_apple) {
			if (.bisearch(ucs, .not_apple, .not_apple.length - 1)) {
				return 0;
			}
		}
		...
		return 1 +
				(
					...
					(
						...
						((ucs >= 0xFFE0) && (ucs <= 0xFFE6)) || ((ucs >= 0x020000) && (ucs <= 0x02FFFD)) || ((ucs >= 0x030000) && (ucs <= 0x03FFFD))
					)
				);
	}
 ...
 */
private static immutable .interval[] ambiguous =
[
	...
	{0xFFFD, 0xFFFD}, {0x0F0000, 0x0FFFFD}, {0x100000, 0x10FFFD},
];

/* For Japanese legacy encodings, the following characters are added. */
private static immutable .interval[] legacy_ja = [{0x00A2, 0x00A3}, {0x00A5, 0x00A6}, {0x00AC, 0x00AC}, {0x00AF, 0x00AF}, {0x2212, 0x2212}];

private static immutable .interval[] emoji =
[
	{0x2600, 0x27BF},
	{0x01F300, 0x01F64F},
	{0x01F680, 0x01F6FF},
...
public int wcwidth_cjk_compat(uint ucs, scope const wcwidth_cjk_compat_option* option = default_wcwidth_cjk_compat_option_ptr)
	...
	{
		/* binary search in table of non-spacing characters */
		if (.bisearch(ucs, .ambiguous, .ambiguous.length - 1)) {
			return 2;
		}

		if (option.is_legacy_ja) {
			if (.bisearch(ucs, .legacy_ja, .legacy_ja.length - 1)) {
				return 2;
			}
		}

		if (option.is_emoji) {
			if (.bisearch(ucs, .emoji, .emoji.length - 1)) {
				return 2;
			}
		}
